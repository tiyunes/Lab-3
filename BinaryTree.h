...
public:
    ...
    BinaryTreeNode<T>* Find(T item)
    {
        ...
        {
            return this->root->GetRight()->Find(item);
        }
        else if (this->root->Get() > item)
        {
            return this->root->GetLeft()->Find(item);
        }
    }
    ...
    void Remove (T item)
    {
        ...
        else Remove(item, Find(item));
    }
    void Remove(T item, BinaryTreeNode<T>* node)
    {
        if (node->Get() == item && this->root->Get() != item)
        {
            node->RemoveNode(node, this->root->Previous(node->Get()));
            node = nullptr;
            delete node;
        }
        else if (this->root->Get() == item)
    ...
    BinaryTree<T>* SubTree(BinaryTreeNode<T>* n)
    {
        BinaryTree<T>* subtree = new BinaryTree<T>();
        if (Find(n->Get()) != nullptr)
        {
            subtree->Add(n->Get());
            return subtree;
        }
